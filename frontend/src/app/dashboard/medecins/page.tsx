...
export default function MedecinsPage() {
  ...
  const [modal, setModal] = useState<{ open: boolean; item?: Medecin }>({ open: false });
  const [confirmDelete, setConfirmDelete] = useState<Medecin | null>(null);
  const [deleting, setDeleting] = useState(false);

  const load = useCallback(async () => {
    setLoading(true);
    try { const data = await api.get<Medecin[]>('/api/medecins/'); setItems(data); setFiltered(data); }
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try { await api.delete(`/api/medecins/${confirmDelete.id}/`); setConfirmDelete(null); load(); }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-800">Médecins</h1>
          <p className="text-slate-500 text-sm mt-0.5">{filtered.length} médecin{filtered.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16 text-slate-500"><p className="font-medium">Aucun médecin trouvé</p></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50">
                  <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Médecin</th>
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Adresse</th>
                    ...
                    <tr key={m.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-rose-100 flex items-center justify-center text-rose-700 font-semibold text-xs shrink-0">{name[0]?.toUpperCase()}</div>
                          <span className="font-medium text-slate-800">{name}</span>
                        </div>
                      </td>
                      <td className="px-5 py-3.5 text-slate-600">{m.user.email}</td>
                      <td className="px-5 py-3.5 text-slate-600">{m.user.telephone || '—'}</td>
                      <td className="px-5 py-3.5 text-slate-600">{m.user.adresse || '—'}</td>
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2 justify-end">
                          <button onClick={() => setModal({ open: true, item: m })} className="btn-ghost text-xs py-1.5 px-2.5 text-slate-500 hover:text-indigo-600">
                          ...
                          <button onClick={() => setConfirmDelete(m)} className="btn-ghost text-xs py-1.5 px-2.5 text-slate-500 hover:text-red-600">
      ...
      <Modal open={modal.open} title={modal.item ? 'Modifier le médecin' : 'Ajouter un médecin'} onClose={() => setModal({ open: false })}>
        <PersonnelForm type="medecins" item={modal.item} onSuccess={() => { setModal({ open: false }); load(); }} onCancel={() => setModal({ open: false })} />
      </Modal>

      <Modal open={!!confirmDelete} title="Confirmer la suppression" onClose={() => setConfirmDelete(null)} size="sm">
        <p className="text-slate-600 text-sm mb-6">Supprimer <strong>Dr. {confirmDelete?.user.first_name} {confirmDelete?.user.last_name}</strong> ? Action irréversible.</p>
