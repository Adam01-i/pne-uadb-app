...
export default function DossierPage() {
  ...
  async function handleDownloadCertificat(dossierId: number) {
    setDownloading(true);
    setError('');
    try {
      ...
      const res = await fetch(
        `${apiBase}/api/inscriptions/dossiers/${dossierId}/telecharger-certificat/`,
        {
          method: 'GET',
      ...
      link.href = url;
      link.download = `certificat-inscription-${dossierId}.txt`;
  ...
  if (isEtudiant) {
    ...
    return (
      <div className="max-w-2xl">
        ...
        <div className="mb-6">
          {dossier.certificat_disponible ? (
            <button
              onClick={() => handleDownloadCertificat(dossier.idDossier)}
              disabled={downloading}
              className="btn-primary"
            >
              {downloading ? 'Téléchargement...' : "Télécharger mon certificat d'inscription"}
            </button>
          ) : (
            <div className="card p-4 text-sm text-slate-600">
              Le certificat d&apos;inscription sera disponible une fois la visite médicale, la validation bibliothèque et le paiement validés.
            </div>
          )}
        </div>
